...
public class BeaconOfHope extends StatusEffect {
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        ...
        switch (instance.getDuration()) {
            case 680:
                Dialogue.getGumasDialogue("\"정말... 나를 죽일 수 있다고 생각해..?\"").sendServer(player);
                break;

            case 580:
                Dialogue.getGumasDialogue("\"너는 내 새로운 왕께 매우 유용할 거다.\"").sendServer(player);
                break;

            case 480:
                Dialogue.getGumasDialogue("\"그럴려면 너는 먼저 죽어야 해...\"").sendServer(player);
                ...
                if (player.getWorld().getRegistryKey().getValue().equals(World.OVERWORLD.getValue())) {
                    ...
                    if (server != null) {
                        if (glassocean != null) {
                            ...
                            player.sendMessage(Text.translatable("prominent.gumascheck").setStyle(Style.EMPTY.withColor(Formatting.GOLD)), true);
                        }
                    }
                }
                break;
            ...
            case 300:
                Dialogue.getGumasDialogue("\"너는 내가 죽었다고 생각했겠지...\"").sendServer(player);
                break;
            case 220:
                Dialogue.getGumasDialogue("\"황색 왕이 나를 더 강하게 다시 태어나게 했주셨다.\"").sendServer(player);
                break;
            case 140:
                Dialogue.getGumasDialogue("\"이제는 네 차례다. 너도 다시 태어나라!\"", 100).sendServer(player);
                break;
            case 80:
    ...
    public void onApplied(LivingEntity entity, AttributeContainer attributes, int amplifier) {
        ...
        if (entity instanceof ServerPlayerEntity player && !entity.getWorld().isClient()) {
           ...
           if (advancement != null) {
                ...
                if (!progress.isDone()) {
                    player.sendMessage(Text.translatable("prominent.gumasnotyet").setStyle(Style.EMPTY.withColor(Formatting.RED)), false);
        ...
        if (!entity.getWorld().getRegistryKey().getValue().equals(World.OVERWORLD.getValue())) {
            entity.sendMessage(Text.translatable("prominent.gumasnotyet2").setStyle(Style.EMPTY.withColor(Formatting.RED)));
