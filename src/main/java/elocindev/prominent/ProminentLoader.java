...
public class ProminentLoader implements ModInitializer {
	...
	public static final ItemGroup ProminentTab = FabricItemGroup.builder()
        ...
        .displayName(Text.empty().append(Text.literal("\uF935 ").setStyle(Style.EMPTY.withColor(Formatting.WHITE))).append(TextAPI.Styles.getStaticGradient(Text.literal("신화 보스"), 0x883db8, 0xac54b3)))
	...
	public void onInitialize() {
		...
		ItemGroupEvents.modifyEntriesEvent(RegistryKey.of(RegistryKeys.ITEM_GROUP, new Identifier(MODID, "tab"))).register(content -> {
			...
			content.add(ItemRegistry.EKAVAR.getDefaultStack());
			content.add(Ekavar.setPhase(ItemRegistry.EKAVAR.getDefaultStack(), 1));
			content.add(Ekavar.setPhase(ItemRegistry.EKAVAR.getDefaultStack(), 2));
			content.add(Ekavar.setPhase(ItemRegistry.EKAVAR.getDefaultStack(), 3));
			content.add(Ekavar.setPhase(ItemRegistry.EKAVAR.getDefaultStack(), 4));
			content.add(Ekavar.setPhase(ItemRegistry.EKAVAR.getDefaultStack(), 5));
		});
